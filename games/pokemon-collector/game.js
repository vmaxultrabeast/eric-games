...
let gameState = {
    ...
    hasJonathanHelper: false,
    lastHelperPokeTime: 0
};
    ...
    if (saved) {
        try {
            ...
            gameState = {
                ...
                hasJonathanHelper: parsed.hasJonathanHelper || false,
                lastHelperPokeTime: parsed.lastHelperPokeTime || 0
            };

            // Check offline helper progress
            if (gameState.hasJonathanHelper && gameState.lastHelperPokeTime > 0) {
                const now = Date.now();
                const elapsed = now - gameState.lastHelperPokeTime;
                const offlineInterval = 30 * 60 * 1000; // 30 minutes in ms
                const pokemonGained = Math.floor(elapsed / offlineInterval);
                if (pokemonGained > 0) {
                    for (let i = 0; i < pokemonGained; i++) {
                        const randomPokemon = ALL_POKEMON_FLAT[Math.floor(Math.random() * ALL_POKEMON_FLAT.length)];
                        if (!gameState.inventory[randomPokemon.name]) {
                            gameState.inventory[randomPokemon.name] = 0;
                        }
                        gameState.inventory[randomPokemon.name]++;
                    }
                    gameState.lastHelperPokeTime += pokemonGained * offlineInterval;
                    saveState();
                    
                    // Delay alert slightly so page has loaded
                    setTimeout(() => {
                        alert("Helper Jonathan collected " + pokemonGained + " Pokemon while you were offline!");
                    }, 500);
                }
            }
        } catch (e) {
...
function updateRunningTimers() {
    ...
    // Check Jonathan Helper (online mode: gives a pokemon every 30s online)
    if (gameState.hasJonathanHelper) {
        if (!gameState.lastHelperPokeTime) {
            gameState.lastHelperPokeTime = now;
            saveState();
        } else {
            const elapsed = now - gameState.lastHelperPokeTime;
            const onlineInterval = 30 * 1000; // 30 seconds in ms
            if (elapsed >= onlineInterval) {
                // Give random Pokemon
                const randomPokemon = ALL_POKEMON_FLAT[Math.floor(Math.random() * ALL_POKEMON_FLAT.length)];
                if (!gameState.inventory[randomPokemon.name]) {
                    gameState.inventory[randomPokemon.name] = 0;
                ...
                // If currently on the collection screen, refresh it
                const activeTab = document.querySelector(".tab-btn.active");
                if (activeTab && activeTab.getAttribute("data-tab") === "collection") {
                    renderCollectionList();
                }

                // Show a clean brief message in the cheat feedback panel (no emojis)
                const feedback = document.getElementById("cheatFeedback");
                if (feedback) {
                    feedback.textContent = `Helper Jonathan found: ${randomPokemon.name}!`;
                    feedback.style.color = "#4ade80";
                    setTimeout(() => {
                        if (feedback.textContent.startsWith("Helper Jonathan found:")) {
...
function checkTradeOffer() {
    ...
    if (!gameState.tradeOffer || now >= gameState.tradeOffer.expiresAt) {
        ...
        // Cost equals worth * 100
        const cost = RARITY_WORTH[randomPokemon.rarity] * 100;
...
function executeTradePurchase() {
    ...
    // Reset cooldown to 0s by clearing offer and generating new one immediately
...
function applyCheatCode() {
    ...
    if (code === "JONATHAN") {
        if (gameState.hasJonathanHelper) {
            feedback.textContent = "JONATHAN HELPER IS ALREADY ACTIVE!";
            feedback.style.color = "var(--color-cyan)";
            return;
        }
        gameState.hasJonathanHelper = true;
        gameState.lastHelperPokeTime = Date.now();
        saveState();
        feedback.textContent = "JONATHAN HELPER ACTIVATED!";
        feedback.style.color = "#4ade80";
        return;
    }
